		...
		switch ( $type ) {
			case 'search':
				...
				$search_response = wp_remote_get(
					$base_url . '/web/search/topsearch/?' . $query,
					[
						'timeout' => 10,
					]
				);
			...
			case 'request':
				...
				$response  = wp_remote_get( $base_url . '/graphql/query/?' . $query, $args );
				return $response;
				break;
			default:
				$user = jnews_get_instagram_data( $username, 'search' );

				if ( isset( $user['is_private'] ) && $user['is_private'] ) {
					// can change to warning message
					return [];
				}
				$args = [
					'id'         => $user['id'],
