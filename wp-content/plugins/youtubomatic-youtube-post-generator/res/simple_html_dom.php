 ...
 * @subpackage youtubomatic_simple_html_dom
 */
...
function youtubomatic_file_get_html($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	// Ensure maximum length is greater than zero
	if($maxLen <= 0) { $maxLen = MAX_FILE_SIZE; }

	// We DO force the tags to be terminated.
	$dom = new youtubomatic_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
function youtubomatic_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	$dom = new youtubomatic_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
function youtubomatic_dump_html_tree($node, $show_attr=true, $deep=0)
{
	$node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class youtubomatic_simple_html_dom_node
{
	/**
	 * Node type
	 *
	 ...
	 *
	 * Adds itself to the list of DOM Nodes {@see youtubomatic_simple_html_dom::$nodes}
	 */
 ...
 * @package PlaceLocalInclude
 */
class youtubomatic_simple_html_dom
{
	/**
	 ...
	 * string length of ({@see youtubomatic_simple_html_dom::$doc}).
	 *
	 ...
	 * Current character
	 *
	 * Holds the current character at position {@see youtubomatic_simple_html_dom::$pos} in
	 * the document {@see youtubomatic_simple_html_dom::$doc}
	 *
	...
	protected function prepare($str, $lowercase=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new youtubomatic_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new youtubomatic_simple_html_dom_node($this);
			++$this->cursor;
	 ...
	 *
	 * Noise is stored to {@see youtubomatic_simple_html_dom::$noise}
	 *
	 ...
	 *
	 * Noise is restored from {@see youtubomatic_simple_html_dom::$noise}
	 *
	...
	function createElement($name, $value=null) {return youtubomatic_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return end(youtubomatic_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
