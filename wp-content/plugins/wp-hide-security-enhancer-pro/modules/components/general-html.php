            ...
            public static function remove_html_comments($buffer)
                {
                    ...
                    * 
                    * Disabled, first extract the js code then process
                    * <script(?:\b[^>]*?>)([\s\S]*?)<(?:[\s\/]+)script>
                    */
            ...
            function _init_remove_html_new_lines($saved_field_data)
                {
                    ...
                    //do not run when within admin
                    if( defined('WP_ADMIN') &&  ( !defined('DOING_AJAX') ||  ( defined('DOING_AJAX') && DOING_AJAX === FALSE )) && ! apply_filters('wph/components/force_run_on_admin', FALSE, 'remove_html_new_lines' ) )
                        return;
