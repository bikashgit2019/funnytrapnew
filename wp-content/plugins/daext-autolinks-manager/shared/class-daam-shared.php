...
class Daam_Shared
{
    ...
    private function __construct()
    {
        ...
        load_plugin_textdomain('daam', false, 'daext-autolinks-manager/lang/');
		...
		$this->data['options'] = [
			...
			//Tracking -------------------------------------------------------------------------------------------------
			$this->get('slug') . '_tracking_enable_click_tracking' => "1",
			$this->get('slug') . '_tracking_minimum_interval' => "10",

			//Capabilities ---------------------------------------------------------------------------------------------
			$this->get('slug') . '_capabilities_editor_sidebar' => "edit_others_posts",
			$this->get('slug') . '_capabilities_statistics_menu' => "edit_others_posts",
			$this->get('slug') . '_capabilities_wizard_menu' => "manage_options",
			$this->get('slug') . '_capabilities_autolinks_menu' => "edit_others_posts",
			$this->get('slug') . '_capabilities_categories_menu' => "edit_others_posts",
			$this->get('slug') . '_capabilities_term_groups_menu' => "edit_others_posts",
			$this->get('slug') . '_capabilities_tracking_menu' => "edit_others_posts",
			$this->get('slug') . '_capabilities_import_menu' => "manage_options",
			$this->get('slug') . '_capabilities_export_menu' => "manage_options",
			$this->get('slug') . '_capabilities_maintenance_menu' => "manage_options",
			...
			$this->get('slug') . '_pagination_tracking_menu' => "10",

			//Advanced -------------------------------------------------------------------------------------------------
			$this->get('slug') . '_advanced_enable_autolinks' => "1",
			$this->get('slug') . '_advanced_filter_priority' => "-2147483648",
			...
			$this->get('slug') . '_advanced_wizard_rows' => "500",
    ...
    public function add_autolinks($content, $check_query = true, $post_type = '', $post_id = false)
    {
         ...
         */
        $enable_autolinks = get_post_meta($this->post_id, '_daam_enable_autolinks', true);
    ...
    /*
     * Get the number of clicks performed on the autolinks of a specific specific post.
     *
     * @param $post_id The post_id for which the clicks should be counted
     * @return int The number of clicks
     */
    public function get_number_of_auto_links_clicks($post_id)
    {

        global $wpdb;
        $table_name       = $wpdb->prefix . $this->get('slug') . "_tracking";
        $safe_sql         = $wpdb->prepare("SELECT COUNT(*) FROM $table_name WHERE post_id = %d", $post_id);
        $number_of_clicks = $wpdb->get_var($safe_sql);
    ...
    /*
     * True is returned if there is at least one record in the "tracking" db table, otherwise False is returned.
     *
     * @return Bool
     */
    public function is_tracking_empty()
    {
    ...
    /*
     * Adds an hidden input used to store the post id at the end of the content.
     *
     * @param $content
    ...
    public function add_hidden_input($content)
    {
        ...
        $hidden_input = '<input id="daam-post-id" type="hidden" value="' . $this->post_id . '">';
