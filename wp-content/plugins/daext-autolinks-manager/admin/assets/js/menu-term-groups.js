(function($) {
  ...
  $(document).ready(function() {
    ...
    $('.post-type').on('change', function() {
      ...
      var data = {
        'action': 'daam_get_taxonomies',
        'security': window.daamNonce,
        'post_type': postType,
      };

      //send ajax request
      $.post(window.daamAjaxUrl, data, function(data) {
    ...
    $('.menu-icon.delete').click(function(event) {
      ...
      window.DAAM.autolinkToDelete = $(this).prev().val();
  ...
  //         $('#form-delete-' + window.DAAM.autolinkToDelete).submit();
  //       },
