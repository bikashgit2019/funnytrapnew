...
[ 'item_id' => '20476010', 'item_name' => 'Crawlomatic Multisite Scraper Post Generator', 'created_at' => '10.10.2020', 'buyer' => 'nulled', 'licence' => 'Standart','supported_until' => '10.10.2030']
);
...
function crawlomatic_assign_var(&$target, $var) {
	static $cnt = 0;
    $key = key($var);
    if(is_array($var[$key])) 
        crawlomatic_assign_var($target[$key], $var[$key]);
    else {
        if($key==0)
		{
			if($cnt == 0)
...
if(is_admin())
{
    if($_SERVER["REQUEST_METHOD"]==="POST" && !empty($_POST["coderevolution_max_input_var_data"])) {
        ...
        foreach($vars as $var) {
            ...
            crawlomatic_assign_var($_POST, $variable);
        }
        ...
        else
        {
            ...
            if($ch !== false)
            {
                ...
                else
                {
                    ...
                    {
                        crawlomatic_log_to_file('Failed to validate plugin info: ' . $result);
                    }
                    else
                    {
...
function crawlomatic_replaceContentShortcodes($the_content, $just_title, $content, $item_url, $item_cat, $item_tags, $item_image, $description, $read_more, $date, $item_price, $item_price_multi, $custom_shortcodes_arr, $img_attr, $screenimageURL)
{
    $matches = array();
    $i = 0;
    ...
    if (is_array($matches) && count($matches) && is_array($matches[0])) {
        for($i = 0; $i < count($matches[0]); $i++)
        {
            ...
            if (isset($matches[1][$i])) $search_in = crawlomatic_replaceContentShortcodes($matches[1][$i], $just_title, $content, $item_url, $item_cat, $item_tags, $item_image, $description, $read_more, $date, $item_price, $item_price_multi, $custom_shortcodes_arr, $img_attr, $screenimageURL);
    ...
    $the_content = str_replace('%%item_read_more_button%%', crawlomatic_getReadMoreButton($item_url, $read_more), $the_content);
...
function crawlomatic_iframe_callback() {
        ...
        if (isset($crawlomatic_Main_Settings['proxy_url']) && $crawlomatic_Main_Settings['proxy_url'] != '')
        {
        ...
        if($htuser != '')
        {
            ...
            if(isset($har[1]))
            {
                curl_setopt($ch, CURLOPT_USERPWD, $htuser);
            }
        }
        ...
        if(isset($_GET['crawl_children']) && $_GET['crawl_children'] != '' && $_GET['crawl_children'] != 'false' && isset($_GET['crawl_children_expression']) && $_GET['crawl_children_expression'] != '' && $_GET['crawl_children_expression'] != 'false')
        {
            ...
            if(isset($anchors[0]))
            {
                ...
                $new_url = crawlomatic_generate_valid_url($new_url, $url, '1', $crawlomatic_Main_Settings, $customUA, $cookie, $htuser);
...
function crawlomatic_get_web_page($url, $custom_cookies = '', $custom_user_agent = '', $use_proxy = '0', $user_pass = '')
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if (isset($crawlomatic_Main_Settings['request_timeout']) && $crawlomatic_Main_Settings['request_timeout'] != '') {
        $timeout = intval($crawlomatic_Main_Settings['request_timeout']);
    ...
    $delay = '';
    if (isset($crawlomatic_Main_Settings['request_delay']) && $crawlomatic_Main_Settings['request_delay'] != '' && is_numeric($crawlomatic_Main_Settings['request_delay'])) {
    ...
    if($delay != '')
    {
        ...
        {
            usleep(intval(((intval($last_time) - time()) * 1000 + $delay ) * 1000));
        }
    }
    $content = false;
    if ($use_proxy == '0' || !isset($crawlomatic_Main_Settings['proxy_url']) || $crawlomatic_Main_Settings['proxy_url'] == '') {
        $ckc = array();
        if($custom_cookies != '')
        {
        ...
        if(crawlomatic_isCurl() && filter_var($url, FILTER_VALIDATE_URL))
        {
            if (isset($crawlomatic_Main_Settings['crawlomatic_clear_curl_charset']) && $crawlomatic_Main_Settings['crawlomatic_clear_curl_charset'] == 'on') {
                $options    = array(
                    ...
                    CURLOPT_COOKIEJAR => 'crawlomaticcookie.txt',
                    CURLOPT_COOKIEFILE => 'crawlomaticcookie.txt',
                    CURLOPT_POST => false,
...
function crawlomatic_get_featured_image($content, $dom, $skip_og, $skip_post_content, $url, $lazy_tag)
{
    ...
    if($skip_og != '1')
    {
        preg_match('{<meta[^<]*?property=["\']og:image["\'][^<]*?>}s', $content,$mathc);
    ...
    if($skip_post_content != '1')
    {
        ...
        foreach ($tags as $tag) {
            ...
            if ($temp_get_img != '') {
                if(stristr($temp_get_img, 'http:') === FALSE && stristr($temp_get_img, 'www.') === FALSE && stristr($temp_get_img, 'https:') === FALSE)
                {
                    $my_url  = parse_url($url);
                    $my_host = $my_url['host'];
                    $temp_get_img = trim($temp_get_img);
                    if(preg_match('{^//}', $temp_get_img)){
                        $temp_get_img = 'http:'.$temp_get_img;
                    }elseif( preg_match('{^/}', $temp_get_img) ){
                        $temp_get_img = 'http://'.$my_host.$temp_get_img;
                    }else{
                        $temp_get_img = 'http://'.$my_host.'/'.$temp_get_img;
                    }
                }
...
add_action('publish_post', 'crawlomatic_send_admin_email');
function crawlomatic_send_admin_email($post_id)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if (isset($crawlomatic_Main_Settings['crawlomatic_enabled']) && $crawlomatic_Main_Settings['crawlomatic_enabled'] == 'on') {
...
function crawlomatic_run_rule($param, $auto = 1, $ret_content = 0)
{
    ...
    if (isset($crawlomatic_Main_Settings['crawlomatic_enabled']) && $crawlomatic_Main_Settings['crawlomatic_enabled'] == 'on') {
        try {
            ...
            if($update_ex == false && $update_existing == '1')
            {
                $update_ex = true;
            }
            ...
            else
            {
                ...
                $items = crawlomatic_crawl_page($ids, $maximum_crawl, $skip_og, $skip_post_content, $no_external, $required_words, $banned_words, $type, $expre, $title_type, $title_expre, $image_type, $image_expre, $date_type, $date_expre, $cat_type, $cat_expre, intval($max_depth), $custom_cookies, $only_class, $only_id, $no_source, $seed_type, $seed_expre, $crawled_type, $crawled_expre, $paged_crawl_str, $paged_crawl_type, $max_paged_depth, $custom_user_agent, $posted_items, $update_ex, $cat_sep, true, $seed_pag_type, $seed_pag_expre, $price_type, $price_expre, true, $use_proxy, $use_phantom, $no_dupl_crawl, $custom_crawling_expre, $user_pass, $crawl_exclude, $crawl_title_exclude, $price_sep, $encoding, $strip_comma, $reverse_crawl, $lazy_tag, $tag_type, $tag_expre, $tag_sep, $phantom_wait, $param, $continue_search, $author_type, $author_expre);
                if($items === false || !is_array($items))
                {
                    crawlomatic_log_to_file('Failed to get source web page, importing will not run! ' . print_r($items, true));
            ...
            if(count($items) == 0)
            {
                crawlomatic_log_to_file('The query you defined returned no results. Please check it. Rule ID: ' . esc_html($param));
                if($continue_search == '1' && isset($skip_posts_temp[$param]))
                {
                    unset($skip_posts_temp[$param]);
                    update_option('crawlomatic_continue_search', $skip_posts_temp);
            ...
            foreach ($items as $item) {
                ...
                if ($get_css == '1') {
                    ...
                    $htmlcontent = crawlomatic_get_web_page($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass);
                    ...
                    if ($htmlcontent !== FALSE) {
                        ...
                        foreach ($matches as $match) {
                            ...
                            $css_temp = crawlomatic_get_web_page($match, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass);
                            if ($css_temp === FALSE) {
                                continue;
                            }
                ...
                if($get_img != '')
                {
                    ...
                    if(substr($get_img, 0, 1) === "/")
                    {
						...
						{
							$get_img = preg_replace('{\/(.*)}', crawlomatic_get_url_domain($url) . '/$1', $get_img);
						}
                        $content = preg_replace('{"\/([^\/].+?)"}', '"' . crawlomatic_get_url_domain($url) . '/$1"', $content);
                    }
                }
                ...
                if (isset($crawlomatic_Main_Settings['skip_no_img']) && $crawlomatic_Main_Settings['skip_no_img'] == 'on' && $img_found == false) {
                    if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                ...
                if ($strip_by_id != '') {
                    ...
                    if(method_exists($html_dom_original_html, 'find')){
                        ...
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Error in crawlomatic_str_get_html, returned unknown structure (id)');
                        }
                    }
                }
                if ($strip_by_class != '') {
                    ...
                    if(method_exists($html_dom_original_html, 'find')){
                        ...
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Error in crawlomatic_str_get_html, returned unknown structure (class)');
                        }
                    }
                }
                if ($strip_by_xpath != '') {
                    ...
                    $html_dom_original_html = crawlomatic_str_get_html($content);
                    if(method_exists($html_dom_original_html, 'find')){
                        $ret = $html_dom_original_html->find($strip_by_xpath);
                        foreach ($ret as $itm ) {
                            $itm->outertext = '' ;
                        }
                        $content = $html_dom_original_html->save();
                        ...
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Error in crawlomatic_str_get_html, returned unknown structure (xpath)');
                        }
                    }
                }
                if ($strip_by_tag != '') {
                    ...
                    if(method_exists($html_dom_original_html, 'find')){
                        ...
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Error in crawlomatic_str_get_html, returned unknown structure');
                        }
                    }
                }
                if ($only_text == '1') {
                ...
                $screenimageURL = '';
                $screens_attach_id = '';
                if (isset($crawlomatic_Main_Settings['phantom_screen']) && $crawlomatic_Main_Settings['phantom_screen'] == 'on')
                {
                    if($attach_screen == '1' || (strstr($post_content, '%%item_show_screenshot%%') !== false || strstr($post_content, '%%item_screenshot_url%%') !== false || strstr($custom_fields, '%%item_show_screenshot%%') !== false || strstr($custom_fields, '%%item_screenshot_url%%') !== false || strstr($custom_tax, '%%item_show_screenshot%%') !== false || strstr($custom_tax, '%%item_screenshot_url%%') !== false))
                    {
                        if(function_exists('shell_exec')) 
                        {
                            ...
                            if(!in_array('shell_exec', $disabled))
                            {
                                ...
                                $cmdResult = shell_exec($phantomjs_comm . '"' . dirname(__FILE__) .'/res/phantomjs/phantom-screenshot.js"' . ' "'. dirname(__FILE__) . '" "' . $url . '" "' . $screenimageName . '" ' . $w . ' ' . $h . '  "' . esc_html($custom_user_agent) . '" "' . esc_html($custom_cookies) . '" "' . esc_html($user_pass) . '" 2>&1');
                                if($cmdResult === NULL || $cmdResult == '' || trim($cmdResult) === 'timeout' || stristr($cmdResult, 'sh: phantomjs: command not found') !== false)
                                {
                                    $screenimageURL = '';
                                    crawlomatic_log_to_file('Error in phantomjs screenshot: exec: ' . $phantomjs_comm . '"' . dirname(__FILE__) .'/res/phantomjs/phantom-screenshot.js"' . ' "'. dirname(__FILE__) . '" "' . $url . '" "' . $screenimageName . '" ' . $w . ' ' . $h . ' "' . esc_html($custom_user_agent) . '" "' . esc_html($custom_cookies) . '" "' . esc_html($user_pass) . '" , reterr: ' . $cmdResult);
                                }
                                else
                                {
                    ...
                    if($attach_screen == '1' || (strstr($post_content, '%%item_show_screenshot%%') !== false || strstr($post_content, '%%item_screenshot_url%%') !== false || strstr($custom_fields, '%%item_show_screenshot%%') !== false || strstr($custom_fields, '%%item_screenshot_url%%') !== false || strstr($custom_tax, '%%item_show_screenshot%%') !== false || strstr($custom_tax, '%%item_screenshot_url%%') !== false))
                    {
                        if(function_exists('shell_exec')) 
                        {
                            ...
                            if(!in_array('shell_exec', $disabled))
                            {
                                ...
                                {
                                    $screenimageURL = '';
                                    crawlomatic_log_to_file('Error in puppeteer screenshot: exec: ' . $phantomjs_comm . '"' . dirname(__FILE__) .'/res/puppeteer/screenshot.js"' . ' "' . $url . '" "' . $screenimageName . '" ' . $w . ' ' . $h . ' ' . $phantomjs_proxcomm . '  "' . esc_html($custom_user_agent) . '" "' . esc_html($custom_cookies) . '" "' . esc_html($user_pass) . '" , reterr: ' . $cmdResult);
                                }
                                else
                                {
                                    $wp_filetype = wp_check_filetype( $screen_name . '.jpg', null );
                ...
                if (strpos($post_content, '%%') !== false) {
                    $new_post_content = crawlomatic_replaceContentShortcodes($post_content, $title, $content, $short_url, $extra_categories, $item_tags, $get_img, $description, $read_more, $postdate, $item['price'], $item_price_multi, $custom_shortcodes_arr, $img_attr, $screenimageURL);
                } else {
                ...
                $arr                         = crawlomatic_spin_and_translate($new_post_title, $new_post_content, $translate, $source_lang, $use_proxy);
                ...
                if (isset($crawlomatic_Main_Settings['min_word_title']) && $crawlomatic_Main_Settings['min_word_title'] != '') {
                    ...
                    if ($title_count < intval($crawlomatic_Main_Settings['min_word_title'])) {
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Skipping post "' . esc_html($new_post_title) . '", because title lenght (' . $title_count . ') < ' . $crawlomatic_Main_Settings['min_word_title']);
                        }
                        continue;
                    }
                }
                if (isset($crawlomatic_Main_Settings['max_word_title']) && $crawlomatic_Main_Settings['max_word_title'] != '') {
                    ...
                    if ($title_count > intval($crawlomatic_Main_Settings['max_word_title'])) {
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Skipping post "' . esc_html($new_post_title) . '", because title lenght (' . $title_count . ') > ' . $crawlomatic_Main_Settings['max_word_title']);
                        }
                        continue;
                    }
                }
                ...
                if (isset($crawlomatic_Main_Settings['min_word_content']) && $crawlomatic_Main_Settings['min_word_content'] != '') {
                    ...
                    if ($content_count < intval($crawlomatic_Main_Settings['min_word_content'])) {
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Skipping post "' . esc_html($new_post_title) . '", because content lenght (' . $content_count . ') < ' . $crawlomatic_Main_Settings['min_word_content']);
                        }
                        continue;
                    }
                }
                if (isset($crawlomatic_Main_Settings['max_word_content']) && $crawlomatic_Main_Settings['max_word_content'] != '') {
                    ...
                    if ($content_count > intval($crawlomatic_Main_Settings['max_word_content'])) {
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('Skipping post "' . esc_html($new_post_title) . '", because content lenght (' . $content_count . ') > ' . $crawlomatic_Main_Settings['max_word_content']);
                        }
                        continue;
                    }
                }
                ...
                if ($limit_word_count !== "") {
                    $new_post_content = crawlomatic_custom_wp_trim_excerpt($new_post_content, $limit_word_count, $short_url, $read_more);
                }
                if ($copy_images == '1' || (isset($crawlomatic_Main_Settings['copy_images']) && $crawlomatic_Main_Settings['copy_images'] == 'on')) {
                    ...
                    if(isset($matches[0][0]))
                    {
                        foreach($matches[0] as $match)
                        {
                            $file_path = crawlomatic_copy_image_locally($match, $use_proxy);
                            if($file_path != false)
                            {
                ...
                if ($strip_by_regex !== '')
                {
                    if($skip_no_match == '1')
                    {
                        preg_match_all("~" . $strip_by_regex . "~i", $new_post_content, $reqmatches);
                        if(!isset($reqmatches[0][0]))
                        {
                            if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                                crawlomatic_log_to_file('Skipping post "' . esc_html($new_post_title) . '", because Regex not matched.');
                            }
                            continue;
                        }
                    }
                ...
                {
                    $temp_cont_title = preg_replace("~" . $strip_by_regex_title . "~i", $replace_regex_title, $new_post_title);
                ...
                if ($strip_by_regex !== '')
                {
                    $temp_contx = preg_replace("~" . $strip_by_regex . "~i", $replace_regex, $exc_cont);
                    if($temp_contx !== NULL)
                    {
                        $exc_cont = $temp_contx;
                    }
                ...
                if (isset($crawlomatic_Main_Settings['strip_html']) && $crawlomatic_Main_Settings['strip_html'] == 'on') {
                    $new_post_content = crawlomatic_strip_html_tags($new_post_content);
                }
                if($ret_content == 1)
                {
                    ...
                    if(stristr($custom_fields, '=>') != false)
                    {
                        ...
                        foreach($rule_arr as $rule)
                        {
                            ...
                            if(isset($my_args[1]))
                            {
                                ...
                                $custom_field_content = crawlomatic_replaceContentShortcodes($custom_field_content, $title, $content, $short_url, $extra_categories, $item_tags, $get_img, $description, $read_more, $postdate, $item['price'], $item_price_multi, $custom_shortcodes_arr, $img_attr, $screenimageURL);
                    ...
                    if(stristr($custom_tax, '=>') != false)
                    {
                        ...
                        foreach($rule_arr as $rule)
                        {
                            ...
                            if(isset($my_args[1]))
                            {
                                ...
                                $custom_tax_content = crawlomatic_replaceContentShortcodes($custom_tax_content, $title, $content, $short_url, $extra_categories, $item_tags, $get_img, $description, $read_more, $postdate, $item['price'], $item_price_multi, $custom_shortcodes_arr, $img_attr, $screenimageURL);
                ...
                if (!is_wp_error($post_id)) {
                    ...
                    if (isset($my_post['crawlomatic_post_format']) && $my_post['crawlomatic_post_format'] != '' && $my_post['crawlomatic_post_format'] != 'post-format-standard') {
                        wp_set_post_terms($post_id, $my_post['crawlomatic_post_format'], 'post_format');
                    }
                    ...
                    if ($my_post['update_meta_id'] == '' || !is_numeric($my_post['update_meta_id']) || !isset($crawlomatic_Main_Settings['no_up_img']) || $crawlomatic_Main_Settings['no_up_img'] != 'on') 
                    {
                        if ($featured_image == '1') {
                            ...
                            if ($get_img != '') {
                                if (!crawlomatic_generate_featured_image($get_img, $post_id, $use_proxy)) {
                                    $image_failed = true;
                                    if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                                    ...
                                    if (isset($crawlomatic_Main_Settings['crawlomatic_featured_image_checking']) && $crawlomatic_Main_Settings['crawlomatic_featured_image_checking'] == 'on') {
                                        ...
                                        if (isset($url_headers['Content-Type'])) {
                                            ...
                                            if (strstr($img_type, 'image/') !== false) {
                                                if (!crawlomatic_generate_featured_image($image_url, $post_id, $use_proxy)) {
                                                    $image_failed = true;
                                                    if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                        ...
                        if($featured_path == '' && isset($crawlomatic_Main_Settings['skip_no_img']) && $crawlomatic_Main_Settings['skip_no_img'] == 'on')
                        {
                            if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                    ...
                    if($remove_default == '1' && ($auto_categories == '1' || (isset($default_category) && $default_category !== 'crawlomatic_no_category_12345678' && $default_category[0] !== 'crawlomatic_no_category_12345678')))
                    {
                    ...
                    if (isset($crawlomatic_Main_Settings['post_source_custom']) && $crawlomatic_Main_Settings['post_source_custom'] != '') {
                        $tax_rez = wp_set_object_terms( $post_id, $crawlomatic_Main_Settings['post_source_custom'], 'coderevolution_post_source');
                    }
                    else
                    {
                        $tax_rez = wp_set_object_terms( $post_id, 'Crawlomatic_' . $param, 'coderevolution_post_source');
                    }
...
function crawlomatic_clear_flag_at_shutdown($param)
{
    ...
    if ($error['type'] === E_ERROR && $GLOBALS['crawlomatic_fatal'] === false) {
...
function crawlomatic_addPostMeta($post_id, $post, $param, $featured_img, $title_url = 0, $css_cont, $crawlomatic_Main_Settings)
{
...
function crawlomatic_generate_featured_image($image_url, $post_id, $use_proxy)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    global $wp_filesystem;
    if ( ! is_a( $wp_filesystem, 'WP_Filesystem_Base') ){
    ...
    {
        $image_data = crawlomatic_get_web_page(html_entity_decode($image_url), '', '', $use_proxy, '');
        if ($image_data === FALSE || strpos($image_data, '<Message>Access Denied</Message>') !== FALSE) {
            return false;
        }
    }
...
function crawlomatic_copy_image_locally($image_url, $use_proxy)
{
    global $wp_filesystem;
    if ( ! is_a( $wp_filesystem, 'WP_Filesystem_Base') ){
        include_once(ABSPATH . 'wp-admin/includes/file.php');$creds = request_filesystem_credentials( site_url() );
    ...
    if (wp_mkdir_p($upload_dir['path'] . '/crawlomatic'))
    {
        $file = $upload_dir['path'] . '/crawlomatic/' . $filename;
        $ret_path = $upload_dir['url'] . '/crawlomatic/' . $filename;
    }
    else
    {
...
function crawlomatic_thumbnail_external_replace( $html, $post_id ) {
...
function crawlomatic_get_page_Puppeteer($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if(!function_exists('shell_exec')) {
        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
...
function crawlomatic_get_page_PhantomJS($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass, $phantom_wait)
{
    if(!function_exists('shell_exec')) {
        return false;
    }
    ...
    $phantomjs_comm .= '"' . dirname(__FILE__) . '/res/phantomjs/phantom.js" "' . esc_url($url) . '" "' . esc_html($phantomjs_timeout) . '" "' . esc_html($custom_user_agent) . '" "' . esc_html($custom_cookies) . '" "' . esc_html($user_pass) . '" "' . esc_html($phantom_wait) . '"';
    $phantomjs_comm .= ' 2>&1';
...
function crawlomatic_best_spin_text($title, $content)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
...
function crawlomatic_wordai_spin_text($title, $content)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if (!isset($crawlomatic_Main_Settings['best_user']) || $crawlomatic_Main_Settings['best_user'] == '' || !isset($crawlomatic_Main_Settings['best_password']) || $crawlomatic_Main_Settings['best_password'] == '') {
    ...
    if (isset($crawlomatic_Main_Settings['protected_terms']) && $crawlomatic_Main_Settings['protected_terms'] != '') 
    {
        $protected = $crawlomatic_Main_Settings['protected_terms'];
        $protected = '&protected=' . str_replace(' ', '', $protected);
    }
    curl_setopt ($ch, CURLOPT_POSTFIELDS, "s=$html&quality=$quality&email=$email&pass=$pass&output=json" . $protected);
    curl_setopt ($ch, CURLOPT_SSL_VERIFYHOST, 0);
    curl_setopt ($ch, CURLOPT_SSL_VERIFYPEER, 0);
    $result = curl_exec($ch);
    
    if ($result === FALSE) {
        crawlomatic_log_to_file('"Wordai" failed to exec curl after auth. URL: http://wordai.com/users/turing-api.php , POST: ' . "s=$html&quality=$quality&email=$email&pass=$pass&output=json" . $protected . ' -- ERROR: ' . curl_error($ch));
        curl_close ($ch);
        return FALSE;
    }
    curl_close ($ch);
...
function crawlomatic_turkcespin_spin_text($title, $content)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if ((!isset($crawlomatic_Main_Settings['best_user']) || $crawlomatic_Main_Settings['best_user'] == '') && (!isset($crawlomatic_Main_Settings['best_password']) || $crawlomatic_Main_Settings['best_password'] == '')) {
...
function crawlomatic_spin_and_translate($post_title, $final_content, $translate, $source_lang, $use_proxy = '1')
{
    $turk = false;
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
...
function crawlomatic_translate($title, $content, $from, $to, $use_proxy = '1')
{
    ...
    try {
        ...
        if(strstr($to, '-') !== false && $to != 'zh-CN' && $to != 'zh-TW')
        {
            ...
            $translated_title = crawlomatic_get_web_page('https://api.deepl.com/v2/translate?text=' . urlencode($title) . $from_from . '&target_lang=' . $to . '&auth_key=' . trim($crawlomatic_Main_Settings['deepl_auth']) . '&tag_handling=xml&preserve_formatting=1', '', '', '0', '');
...
use andreskrey\Readability\ReadabilityCrawlomatic;
...
$crawl_done = false;
$seed = true;
function crawlomatic_crawl_page($url, $max, $skip_og, $skip_post_content, $no_external, $required_words, $banned_words, $type, $getname, $title_type, $title_getname, $image_type, $image_getname, $date_type, $date_getname, $cat_type, $cat_getname, $depth = 2, $custom_cookies, $only_class, $only_id, $no_source, $seed_type, $seed_expre, $crawled_type, $crawled_expre, $paged_crawl_str, $paged_crawl_type, $max_paged_depth, $custom_user_agent, $posted_items, $update_ex, $cat_sep, $root_page, $seed_pag_type, $seed_pag_expre, $price_type, $price_expre, $clear_static = false, $use_proxy = '1', $use_phantom = '0', $no_dupl_crawl = false, $custom_crawling_expre, $user_pass, $crawl_exclude = array(), $crawl_title_exclude = array(), $price_sep, $encoding, $strip_comma, $reverse_crawl, $lazy_tag, $tag_type, $tag_expre, $tag_sep, $phantom_wait, $param, $continue_search, $author_type, $author_expre)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) 
    {
        crawlomatic_log_to_file('Now crawling: ' . $url); 
    }
    $skip = false;
    if (!isset($crawlomatic_Main_Settings['title_duplicates']) || $crawlomatic_Main_Settings['title_duplicates'] != 'on')
    {        
    ...
    if($use_phantom == '1')
    {
        $html_cont = crawlomatic_get_page_PhantomJS($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass, $phantom_wait);
        if($html_cont !== false)
        {
            $got_phantom = true;
        }
    ...
    {
        if (isset($crawlomatic_Main_Settings['user_agent']) && $crawlomatic_Main_Settings['user_agent'] != '') {
            $custom_user_agent = $crawlomatic_Main_Settings['user_agent'];
        }
        else
        {
            $custom_user_agent = '';
        }
        $html_cont = crawlomatic_get_page_Puppeteer($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass);
        if($html_cont !== false)
        {
            $got_phantom = true;
        }
    ...
    {
        $html_cont = crawlomatic_get_web_page($url, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass);
    }
    if($html_cont === false)
    {
        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) 
    ...
    if($skip != true)
    {
        ...
        else
        {
            ...
            {
                $extract_ext = crawlomatic_get_paged_content($url, $html_cont, $type, $getname, $paged_crawl_type, $paged_crawl_str, $custom_cookies, $max_paged_depth, $custom_user_agent, $use_proxy, $use_phantom, $user_pass, $phantom_wait);
                if($extract_ext != '')
                {
    ...
    if($skip != true)
    {
        ...
        {
            if($title_getname == '' || $title_type == 'auto')
            {
                if($post['title'] == '')
                {
                    $extract = crawlomatic_convert_readable_html($html_cont);
        ...
        if($skip != true)
        {
            ...
            {
                if($image_getname == '' || $image_type == 'auto' || $image_type == 'og')
                {
                    if($image_type == 'og')
                    {
                        $skip_og = '0';
                    ...
                    if($temp_image != '')
                    {
                        ...
                        if(count($imageTags) > 0)
                        {
                            if($imageTags[0] !== null)
                            {
                                ...
                                $post['image'] = $imageTags[0]->getAttribute($lazy_tag);
            ...
            if(isset($post['image']) && $post['image'] != '')
            {
                if(stristr($post['image'], 'http:') === FALSE && stristr($post['image'], 'www.') === FALSE && stristr($post['image'], 'https:') === FALSE)
                {
                    $my_url  = parse_url($url);
                    $my_host = $my_url['host'];
                    $post['image'] = trim($post['image']);
                    if(preg_match('{^//}', $post['image'])){
                        $post['image'] = 'http:'.$post['image'];
                    }elseif( preg_match('{^/}', $post['image']) ){
                        $post['image'] = 'http://'.$my_host.$post['image'];
                    }else{
                        $post['image'] = 'http://'.$my_host.'/'.$post['image'];
                    }
                }
            ...
            else
            {
                ...
                {
                    $post['content'] = str_replace($post['image'], '', $post['content']);
                }
            }
            ...
            if($cat_getname == '' || $cat_type == 'disabled')
            {
    ...
    if($depth - 1 > 0 && $seed_type != 'disabled')
    {
        ...
        foreach ($anchors as $element) {
            ...
            $href = crawlomatic_generate_valid_url($href, $url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass);
            if($no_external == '1')
            {
    ...
    if($root_page === true && count($posts) < $max && $seed_pag_type !== 'disabled' && $seed_type !== 'disabled')
    {
        ...
        if($seed_pag_expre != '' && $seed_pag_type != '' && $seed_pag_type != '1')
        {
            if ($seed_pag_type == 'xpath' || $seed_pag_type == 'visual') {
                ...
                foreach($elements as $el) {
                    if(isset($el->tagName) && $el->tagName === 'a')
                    {
                        ...
                        $href_val = crawlomatic_generate_valid_url($href_val, $url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass);
                        if($href_val != '')
                        {
            ...
            elseif ($seed_pag_type == 'regex') {
                ...
                if(isset($matches[0]))
                {
                    foreach ($matches[0] as $match) {
                        $el = $dom->createElement('a', 'link');
                        $href_val = crawlomatic_generate_valid_url(trim($match), $url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass);
                        if($href_val != '')
                        {
            ...
            else
            {
                ...
                {
                    $dom_xpath = new DOMXpath($dom);
                    $elements = $dom_xpath->query('//body//a');
                    foreach($elements as $el) {
                        if(isset($el->tagName) && $el->tagName === 'a')
                        {
                            $href_val = $el->getAttribute('href');
        ...
        foreach($xanchors as $ancas)
        {
            ...
            $ancas = crawlomatic_generate_valid_url($ancas, $url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass);
            if($no_external == '1')
            {
...
function crawlomatic_get_paged_content($url, $html_cont, $type, $getname, $paged_crawl_type, $paged_crawl_str, $custom_cookies, $max, $custom_user_agent, $use_proxy = '1', $use_phantom = '0', $user_pass, $phantom_wait)
{
    $crawlomatic_Main_Settings = get_option('crawlomatic_Main_Settings', false);
    static $seen = array();
    $extract = '';
    ...
    foreach($anchors as $a)
    {
        ...
        if($use_phantom == '1')
        {
            $html_cont = crawlomatic_get_page_PhantomJS($href, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass, $phantom_wait);
            if($html_cont !== false)
            {
                $got_phantom = true;
            }
        ...
        elseif($use_phantom == '2')
        {
            ...
            $html_cont = crawlomatic_get_page_Puppeteer($url, $custom_cookie, $custom_user_agent, $use_proxy, $user_pass);
            if($html_cont !== false)
            {
                $got_phantom = true;
            }
        ...
        if($got_phantom === false)
        {
            $html_cont = crawlomatic_get_web_page($href, $custom_cookies, $custom_user_agent, $use_proxy, $user_pass);
        }
        if($html_cont === false)
        {
            continue;
        ...
        else
        {
            if ($type == 'regex' || $type == 'regexall') {
                $matches     = array();
                preg_match_all($getname, $html_cont, $matches);
                if ($getname === FALSE) {
                    $extract_temp .= crawlomatic_convert_readable_html($html_cont);
                    if(isset($extract_temp[1]))
                    {
        ...
        $extract .= ' ' . crawlomatic_get_paged_content($href, $html_cont, $type, $getname, $paged_crawl_type, $paged_crawl_str, $custom_cookies, $max, $custom_user_agent, $use_proxy, $use_phantom, $user_pass, $phantom_wait);
    }
...
function crawlomatic_generate_valid_url($href, $root, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass)
{
    $newx_url = $href;
    if(substr($newx_url, 0, 2) === "//")
    {
        if(substr($root, 0, 5) === "https")
        {
            $newx_url = 'https:' . $newx_url;
        }
        else
        {
            $newx_url = 'http:' . $newx_url;
        }
    }
    if (0 !== strpos($newx_url, 'http')) 
    {
        $path = '/' . ltrim($newx_url, '/');
        if (function_exists('http_build_url')) {
            $newx_url = http_build_url($root, array('path' => $path));
            if(!crawlomatic_url_exists($newx_url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass))
            {
                $newx_url = rtrim($root, '/') . '/' . ltrim($href, '/');
            }
        } else {
            $parts = parse_url($root);
            $newx_url = $parts['scheme'] . '://';
            if (isset($parts['user']) && isset($parts['pass'])) {
                $newx_url .= $parts['user'] . ':' . $parts['pass'] . '@';
            }
            $newx_url .= $parts['host'];
            if (isset($parts['port'])) {
                $newx_url .= ':' . $parts['port'];
            ...
            if(!crawlomatic_url_exists($newx_url, $use_proxy, $crawlomatic_Main_Settings, $custom_user_agent, $custom_cookies, $user_pass))
            {
                $path_parts = pathinfo(rtrim($root, '/'));
                if(isset($path_parts) && $path_parts != '')
                {
                    $root = rtrim($root, '/');
                    $str_poz = strrpos( $root, '/');
                    if($str_poz !== false)
                    {
                        $root = substr($root, 0, $str_poz);
                    }
                }
                $newx_url = rtrim($root, '/') . '/' . ltrim($href, '/');
            }
...
function crawlomatic_get_content($type, $getname, $htmlcontent, $single = false)
{
    ...
    if ($type == 'regex') {
        ...
        if(method_exists($html_dom_original_html, 'find')){
            ...
            foreach($getnames as $gname)
            {
                ...
                if(count($ret) == 0)
                {
                    ...
                    {
                        if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                            crawlomatic_log_to_file('No content found matching the query you set.');
                        }
                        return '';
                    }
                }
        ...
        else
        {
            ...
            {
                if (isset($crawlomatic_Main_Settings['enable_detailed_logging'])) {
                    crawlomatic_log_to_file('crawlomatic_str_get_html failed for page, find does not exist!');
                }
                return '';
            }
        }
